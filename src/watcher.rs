...
fn load_cms_file(
    ...
    parsing::parse_templates(&mut cms_file, run_args);
    return Ok(cms_file);
}

fn write_gen_site(file_path: std::path::PathBuf, cms_file: &CMSFile, run_args: &run_args::RunArgs) {
    println!("Writing generated files");
    let gen_file = generate_website(&cms_file, run_args);
...
fn watch_event(_event: notify::Event, run_args: run_args::RunArgs) {
    ...
    let index_file = load_cms_file(run_args.in_source("index.cms"), &run_args)
